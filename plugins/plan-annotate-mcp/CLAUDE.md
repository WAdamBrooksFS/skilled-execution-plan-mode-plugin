...
**plan-annotate-mcp** is a Claude Code plugin that enables proactive MCP (Model Context Protocol) server evaluation during plan mode. When enabled (default state), Claude will check which MCP servers are available in the environment and mention them in plan presentations.
...
3. **Preference Initialization**: Script creates `.claude/preferences.json` if missing and auto-initializes `MCP_PLAN_ANNOTATIONS: true`
...
1. **/planning-mcp-annotations-on** (`commands/planning-mcp-annotations-on.md`)
   - Sets `MCP_PLAN_ANNOTATIONS: true`
   - Creates preferences file if missing
   - Confirms enablement to user

2. **/planning-mcp-annotations-off** (`commands/planning-mcp-annotations-off.md`)
   - Sets `MCP_PLAN_ANNOTATIONS: false`
   - Reverts to default planning behavior
   - Confirms disablement to user

3. **/planning-mcp-annotations-cleanup** (`commands/planning-mcp-annotations-cleanup.md`)
   - Removes `MCP_PLAN_ANNOTATIONS` key entirely
...
# Check .claude/preferences.json - should contain MCP_PLAN_ANNOTATIONS: true

/planning-mcp-annotations-off
# Check .claude/preferences.json - should contain MCP_PLAN_ANNOTATIONS: false

/planning-mcp-annotations-cleanup
# Check .claude/preferences.json - MCP_PLAN_ANNOTATIONS key should be removed
```

### Test Plan Mode Behavior

1. Enable plugin: `/planning-mcp-annotations-on`
...
```json
{
  "name": "plan-annotate-mcp",
  "commands": ["./commands/planning-mcp-annotations-on.md", ...],
