# SessionStart hook for Plan Annotations: MCP (PowerShell version)
...
if ($null -eq $prefs.MCP_PLAN_ANNOTATIONS) {
    # Add the property
    $prefs | Add-Member -MemberType NoteProperty -Name "MCP_PLAN_ANNOTATIONS" -Value $true -Force
...
if ($prefs.MCP_PLAN_ANNOTATIONS -eq $true) {
    Write-Output @"
