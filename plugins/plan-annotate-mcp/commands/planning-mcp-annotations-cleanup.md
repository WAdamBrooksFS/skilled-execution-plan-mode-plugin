Remove all configuration created by the MCP plan annotations plugin. This prepares the system for safe plugin uninstallation.

You should:

1. Check if `.claude/preferences.json` exists
2. Read the current preferences
3. Remove the `MCP_PLAN_ANNOTATIONS` key from the preferences object
...
5. If the object is now empty (no other plugin preferences exist):
   ...
   - Inform the user: "✓ MCP plan annotations configuration has been removed. The preferences file was deleted because it contained no other settings."
6. If other keys remain (other plugins have preferences):
   ...
   - Inform the user: "✓ MCP plan annotations configuration has been removed. Other plugin preferences have been preserved."
