...
If you prefer to clean up manually instead of using the `/planning-skills-annotations-cleanup` command, you have several options:
...
2. Remove the `"SKILLS_PLAN_ANNOTATIONS": true` line
...
**Note:** The `/planning-skills-annotations-cleanup` slash command does NOT require `jq` - it's a Claude Code command that handles cleanup automatically. The jq-based commands above are only needed if you want to bypass the plugin's cleanup command.
