...
# SessionStart hook for Plan Annotations: Agents
...
if ! jq -e '.AGENTS_PLAN_ANNOTATIONS' "$PREFS_FILE" >/dev/null 2>&1; then
  # Add the key with default value (true)
  jq '. + {"AGENTS_PLAN_ANNOTATIONS": true}' "$PREFS_FILE" > "$PREFS_FILE.tmp" 2>/dev/null
  if [ $? -eq 0 ]; then
    mv "$PREFS_FILE.tmp" "$PREFS_FILE"
  fi
fi

# Read the AGENTS_PLAN_ANNOTATIONS value
ENABLED=$(jq -r '.AGENTS_PLAN_ANNOTATIONS // false' "$PREFS_FILE" 2>/dev/null)
