Remove all configuration created by the agents plan annotations plugin. This prepares the system for safe plugin uninstallation.

You should:

1. Check if `.claude/preferences.json` exists
2. Read the current preferences
3. Remove the `AGENTS_PLAN_ANNOTATIONS` key from the preferences object
